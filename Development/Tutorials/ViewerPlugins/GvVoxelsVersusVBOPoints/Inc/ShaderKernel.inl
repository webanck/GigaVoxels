...
inline float ShaderKernel::getConeApertureImpl( const float tTree ) const
{
	...
	return k_renderViewContext.pixelSize.x * tTree * cVoxelScale * k_renderViewContext.frustumNearINV;
}
					...
					const float3 rayDir, float& rayStep, const float coneAperture )
{
	...
	//float4 normal = make_float4( 1.0f, 0.0f, 0.0f, 0.0f );
