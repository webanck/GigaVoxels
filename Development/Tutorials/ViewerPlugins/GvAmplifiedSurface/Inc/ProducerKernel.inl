...
float4 getRGBA( float3 voxelPosF, uint3 levelRes )
{
	...
	{
		dist_noise += cNoiseStrength / frequency * Noise::getValue( frequency * ( voxelPosF - voxelNormalAndDist.w * voxelNormal ) );
	}
...
float3 getNormal( float3 voxelPosF, uint3 levelRes )
{
	...
	for ( float frequency = cNoiseFirstFrequency; frequency < levelRes.x; frequency *= 2.f )
	{
		grad_noise.x +=  cNoiseStrength / frequency * Noise::getValue( frequency * ( voxelPosF + make_float3( eps, 0.0f, 0.0f ) - voxelNormalAndDist.w * voxelNormal ) )
						-cNoiseStrength / frequency * Noise::getValue( frequency * ( voxelPosF - make_float3( eps, 0.0f, 0.0f ) - voxelNormalAndDist.w * voxelNormal ) );

		grad_noise.y +=  cNoiseStrength / frequency * Noise::getValue( frequency * ( voxelPosF + make_float3( 0.0f, eps, 0.0f ) - voxelNormalAndDist.w * voxelNormal ) )
						-cNoiseStrength / frequency * Noise::getValue( frequency * ( voxelPosF - make_float3( 0.0f, eps, 0.0f ) - voxelNormalAndDist.w * voxelNormal ) );

		grad_noise.z +=  cNoiseStrength / frequency * Noise::getValue( frequency * ( voxelPosF + make_float3( 0.0f, 0.0f, eps ) - voxelNormalAndDist.w * voxelNormal ) )
						-cNoiseStrength / frequency * Noise::getValue( frequency * ( voxelPosF - make_float3( 0.0f, 0.0f, eps ) - voxelNormalAndDist.w * voxelNormal ) );
	}
