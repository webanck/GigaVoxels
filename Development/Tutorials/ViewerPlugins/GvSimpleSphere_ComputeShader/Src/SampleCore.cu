...
void SampleCore::init()
{
	...
	// Initialize shader program
	bool status = false;
	_computeShaderProgram = new GvShaderProgram();
	QString computeShaderFilename = dataRepository + QString( "mandelbrot_comp.glsl" );
	status = _computeShaderProgram->addShader( GvShaderProgram::eComputeShader, computeShaderFilename.toStdString() );
	assert( status );
	status = _computeShaderProgram->link();
	assert( status );
	...
	glBindImageTextureEXT( 0, imgTex, 0, GL_FALSE, 0, GL_READ_WRITE, GL_RGBA8 );
	//glBindImageTextureEXT( 1, imgTex, 0, GL_FALSE, 0, GL_READ_WRITE, GL_RGBA8 );
...
void SampleCore::draw()
{
	...
	//	glDisable( GL_DEPTH_TEST );
		...
		//glBindImageTextureEXT( 1, imgTex, 0, GL_FALSE, 0, GL_READ_WRITE, GL_RGBA8 );
		// ----
//		GLint imageLoc = glGetUniformLocation( _computeShaderProgram->_program, "uColorImage" );
//		glUniform1i( imageLoc, 0 );
		//glUniform1i( imageLoc, 1 );
		// ----
		GLint loc;
		loc = glGetUniformLocation( _computeShaderProgram->_program, "uWidth" );
		glUniform1ui( loc, 800 );
		GV_CHECK_GL_ERROR();
		loc = glGetUniformLocation( _computeShaderProgram->_program, "uHeight" );
		glUniform1ui( loc, 600 );
		GV_CHECK_GL_ERROR();
		loc = glGetUniformLocation( _computeShaderProgram->_program, "uComputeSpaceWindow" );
		glUniform4f( loc, -1.0f, -1.0f, 1.0f, 1.0f );
		GV_CHECK_GL_ERROR();
		glDispatchComputeEXT( 256 / 32, 256 / 32, 1 );
		GV_CHECK_GL_ERROR();
