...
#define BRICKPOOL_MEMSIZE	(256U * 1024U * 1024U)		// 128 Mo
...
,	mDisplayPerfmon( 0 )
,	mMaxVolTreeDepth( 0 )
,	_sceneShadowCaster( NULL )
,	_sceneShadowReceiver( NULL )
{
...
void SampleCore::init()
{
	...
	_sceneShadowReceiver = new ObjectReceivingShadow();
	_pipeline->editRenderer()->getVolTreeChildArray()->unmapResource();
	_pipeline->editRenderer()->getVolTreeDataArray()->unmapResource();
	_sceneShadowReceiver->setVolTreeChildArray(_pipeline->editRenderer()->getVolTreeChildArray(), _pipeline->editRenderer()->getChildBufferName());
	_sceneShadowReceiver->setVolTreeDataArray(_pipeline->editRenderer()->getVolTreeDataArray(), _pipeline->editRenderer()->getDataBufferName());
	_sceneShadowReceiver->init();
	_pipeline->editRenderer()->getVolTreeChildArray()->mapResource();
	_pipeline->editRenderer()->getVolTreeDataArray()->mapResource();

	//creating sphere casting the shadow
	_sceneShadowCaster = new Mesh();
	QString sceneShadowCasterFilename = dataRepository + QDir::separator() + QString( "3DModels" ) + QDir::separator() + QString( "sphere.obj" );
	//_sceneShadowCaster->chargerMesh("/home/maverick/bellouki/Gigavoxels/Development/Tutorials/Demos/GraphicsInteroperability/RendererShadow/3DModels/sphere.obj");
	_sceneShadowCaster->chargerMesh( sceneShadowCasterFilename.toStdString() );
	_sceneShadowCaster->creerVBO();
}
...
void SampleCore::draw()
{
	...
	//
	// - 1st, send light position
	_pipeline->editRenderer()->setLightPosition( _lightPosition.x, _lightPosition.y, _lightPosition.z );
	// - then launch GigaVoxels to produce data
	...
	float4x4 TmodelMatrix = transpose( modelMatrix );
	/*cout << "MODELMATRIX" << endl;
	for ( int k = 0; k < 16; k++ )
	{
		cout << TmodelMatrix._array[ k ] << " ";
		if ( k % 4 == 3 )
		{
			cout << endl;
		}
	}*/
	_sceneShadowReceiver->setModelMatrix( modelMatrix._array[ 0 ], modelMatrix._array[ 1 ], modelMatrix._array[ 2 ], modelMatrix._array[ 3 ],  
	...
	_sceneShadowReceiver->setLightPosition( _lightPosition.x, _lightPosition.y, _lightPosition.z);
	_sceneShadowReceiver->setWorldLight( _lightWorldPosition.x, _lightWorldPosition.y, _lightWorldPosition.z );
	uint3 bsc = _pipeline->editRenderer()->getBrickCacheSize();
	_sceneShadowReceiver->setBrickCacheSize( bsc.x, bsc.y, bsc.z );
	float3 bpri = _pipeline->editRenderer()->getBrickPoolResInv();
	_sceneShadowReceiver->setBrickPoolResInv( bpri.x, bpri.y, bpri.z );
	_sceneShadowReceiver->setMaxDepth( _pipeline->editRenderer()->getMaxDepth() );
	GV_CHECK_GL_ERROR();
	_sceneShadowReceiver->render();
	GV_CHECK_GL_ERROR();

	// Render the object casting the shadow
	_sceneShadowCaster->setLightPosition( _lightPosition.x, _lightPosition.y, _lightPosition.z );
	//_sceneShadowCaster->render();
	...
	// Display Performance Monitor info
	if ( mDisplayPerfmon )
	{
...
void SampleCore::setLightPosition( float pX, float pY, float pZ )
{
	...
	/*float3 tnslation = make_float3( -0.5f, -0.5f, -0.5f );
	...
	qglviewer::Vec light = qglviewer::Vec( pX, pY, pZ );
	//cout << pX <<" "<< pY <<" "<< pZ << endl;

	qglviewer::Vec lightV = pipelineViewer->camera()->cameraCoordinatesOf( light );
	qglviewer::Vec wlight = pipelineViewer->camera()->worldCoordinatesOf( lightV );
	_lightPosition = make_float3( lightV.x, lightV.y, lightV.z );
	setWorldLight( wlight.x, wlight.y, wlight.z );
	//cout << "world light : " << wlight.x << " " << wlight.y << " " << wlight.z << endl;
	//---------------------------
}
...
{
	_lightWorldPosition = make_float3( pX, pY, pZ );
}
