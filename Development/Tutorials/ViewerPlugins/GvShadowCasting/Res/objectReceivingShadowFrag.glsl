...
////////////////////////////////////////////////////////////////////////////////
 vec4 sampleBrick( uint brickIdxEnc, vec3 samplePos, vec3 nodePos, float pNodeSize )
 {
	...
	// Sample data
	return texture3D( uDataPool, ( brickPos + samplePosBrick ) * uBrickPoolResInv );
}
...
// @return ...
////////////////////////////////////////////////////////////////////////////////
/*vec4 sampleMipMapInterp( float maxDepthF, float maxDepthNew,
...
{
	//Phong Lighting
	vec3 ambientLight = vec3(0.75); //between 0 and 1
	float shininess = 20;
	vec3 specularColor = vec3(0.7);
	vec4 lcolor = vec4(210.0/255.0, 105.0/255.0, 30.0/255.0, 1.0);
	vec3 pointToLight = vec3(lightPos - pixelPos);
	...
	//Shadow Casting
	vec4 rayTracedColor;
	vec3 localRayDir = normalize(vec3(inverse(modelMatrix)*vec4(worldLight, 1.0)) - vec3(worldPixelPos));
	//vec3 worldRayDir = normalize(vec3(worldLight) - vec3(modelMatrix*worldPixelPos));
	vec3 worldRayDir = normalize(vec3(inverse(modelMatrix)*vec4(worldLight, 1.0)) - vec3(inverse(modelMatrix)*worldPixelPos));
	//vec3 worldRayDir1 = normalize(vec3(worldLight) - vec3(worldPixelPos));
	vec3 boxMin = vec3(0.001, 0.001, 0.001);
	vec3 boxMax = vec3(0.999, 0.999, 0.999 );
	//vec3 boxMin = vec3(modelMatrix*vec4(0.001, 0.001, 0.001, 1.0));
	//vec3 boxMax = vec3(modelMatrix*vec4(0.999, 0.999, 0.999, 1.0));
	//vec3 boxMin = vec3(-0.499, -0.499, -0.499);
	//vec3 boxMax = vec3(0.499, 0.499, 0.499 );
	float tnear, tfar;
	bool hit = intersectBox( vec3(inverse(modelMatrix)*worldPixelPos), worldRayDir, boxMin, boxMax, tnear, tfar );



	//gl_FragColor = worldPixelPos;
	...
	//}
		...
		//vec3 gvPos = vec3(worldPixelPos)+initT*worldRayDir;
		//if (dot(worldRayDir, vec3(worldLight) - gvPos) > 0) {
			//rayTracedColor = traceVoxelConeRayCast( vec3((worldPixelPos*1.75 +1.0)), worldRayDir, initT, coneFactor, tfar );
			rayTracedColor = traceVoxelConeRayCast( vec3(inverse(modelMatrix)*worldPixelPos), worldRayDir, initT, coneFactor, tfar );
			//rayTracedColor = traceVoxelConeRayCast( (vec3(worldPixelPos)-0.5)*0.5+2.0, worldRayDir, initT, coneFactor, tfar );
			if (rayTracedColor.w > 0) {
//				gl_FragColor = mix(vec4(0.1, 0.1, 0.1 ,1.0), litColor, 0.2);
				//gl_FragColor = mix(rayTracedColor, litColor, 0.2);
