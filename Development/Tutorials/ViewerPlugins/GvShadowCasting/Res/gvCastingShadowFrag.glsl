...
void main()
{
	//Computing the pixel 3d position in camera (eye) space
	/*vec4 pixelPos;
	pixelPos.xy = ((2.0 * gl_FragCoord.xy) - (2.0 * viewPort.xy)) / (viewPort.zw) - 1;
	pixelPos.z = (2.0 * gl_FragCoord.z - gl_DepthRange.near - gl_DepthRange.far) / (gl_DepthRange.far - gl_DepthRange.near);
	pixelPos.w = 1.0;

	//Computing the ray from pixel to light source
	vec3 rayDir = normalize(lightPos - vec3(pixelPos)); */
    ...
    //const vec3 boxMin = vec3( 2.501, 2.501, 2.501 );
   // const vec3 boxMax = vec3( 3.499, 3.499, 3.499 );
