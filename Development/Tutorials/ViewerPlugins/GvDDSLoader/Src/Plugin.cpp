 ...
 * 
 ******************************************************************************/
extern "C" GVDDSLOADER_EXPORT GvPluginInterface* createPlugin( GvPluginManager& pManager )
{
...
void GvMyPlugin::initialize()
{
	...
	//	{
			GvvRawDataLoaderDialog dataLoaderDialog( NULL );
			...
			if ( dataLoaderDialog.result() == QDialog::Accepted )
			{
				...
				{
					rawFileReader->setFilename( dataLoaderDialog.get3DModelFilename().toLatin1().constData() );
					rawFileReader->setDataResolution( dataLoaderDialog.get3DModelResolution() );
