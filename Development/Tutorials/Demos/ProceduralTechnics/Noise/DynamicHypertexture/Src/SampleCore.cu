...
#define BRICKPOOL_MEMSIZE	( 128U * 1024U * 1024U )	// 384 Mo
...
void SampleCore::setNoiseParameters( float p1, float p2 )
{
	...
	GV_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cNoiseParameters, &noiseParams, sizeof( noiseParams ), 0, cudaMemcpyHostToDevice ) );
}
...
bool SampleCore::initializePipeline()
{
	...
	_pipeline->editDataStructure()->setMaxDepth( 6 );
