...
// GigaVoxels
#include <GvCore/GvGraphicsCore.h>
...
void SampleViewer::init()
{
	...
	GvCore::GvGraphicsCore::printInfo();
	...
	_light1->setPosition( 16.f, 3.f, 0.f );
	_sampleCore->setLightPosition( 16.f, 3.f, 0.f );
	...
	qglviewer::Vec tmpVector;

	tmpVector = camera()->position();
	_sampleCore->getShadowMap()->setCameraEye( glm::vec3( tmpVector.x, tmpVector.y, tmpVector.z ) );
	tmpVector = camera()->sceneCenter();
	_sampleCore->getShadowMap()->setCameraCenter( glm::vec3( tmpVector.x, tmpVector.y, tmpVector.z ) );
	tmpVector = camera()->upVector();
	_sampleCore->getShadowMap()->setCameraUp( glm::vec3( tmpVector.x, tmpVector.y, tmpVector.z ) );
	_sampleCore->getShadowMap()->setCameraFovY( camera()->fieldOfView() * 180.f / 3.141592f );
	_sampleCore->getShadowMap()->setCameraAspectRatio( camera()->aspectRatio() );
	_sampleCore->getShadowMap()->setCameraZNear( camera()->zNear() );
	_sampleCore->getShadowMap()->setCameraZFar( camera()->zFar() );

	tmpVector = _light1->position();
	_sampleCore->getShadowMap()->setLightEye( glm::vec3( tmpVector.x, tmpVector.y, tmpVector.z ) );
	tmpVector = camera()->sceneCenter();
	_sampleCore->getShadowMap()->setLightCenter( glm::vec3( tmpVector.x, tmpVector.y, tmpVector.z ) );
	tmpVector = camera()->upVector();
	_sampleCore->getShadowMap()->setLightUp( glm::vec3( tmpVector.x, tmpVector.y, tmpVector.z ) );
	_sampleCore->getShadowMap()->setLightFovY( camera()->fieldOfView() * 180.f / 3.141592f );
	_sampleCore->getShadowMap()->setLightAspectRatio( camera()->aspectRatio() );
	_sampleCore->getShadowMap()->setLightZNear( camera()->zNear() );
	_sampleCore->getShadowMap()->setLightZFar( camera()->zFar() );

	// Shadow Map editor
	_shadowMapEditor = new Editor( NULL );
	_shadowMapEditor->initialize( _sampleCore->getShadowMap(), _light1 );
	_shadowMapEditor->show();
...
void SampleViewer::draw()
{
	...
	//// Shadow map
	////_sampleCore->getShadowMap()->setCameraEye( glm::vec3( 0.f, 3.f, 25.f ) );
	//{
	//	qglviewer::Vec tmpVector = camera()->position();
	//	_sampleCore->getShadowMap()->setCameraEye( glm::vec3( tmpVector.x, tmpVector.y, tmpVector.z ) );
	//}
	////_sampleCore->getShadowMap()->setCameraCenter( glm::vec3( 0.f, 3.f, 0.f ) );
	//{
	//	//qglviewer::Vec tmpVector = camera()->sceneCenter();
	//	qglviewer::Vec tmpVector = camera()->position() + camera()->viewDirection();
	//	_sampleCore->getShadowMap()->setCameraCenter( glm::vec3( tmpVector.x, tmpVector.y, tmpVector.z ) );
	//}
	////_sampleCore->getShadowMap()->setCameraUp( glm::vec3( 0.f, 1.f, 0.f ) );
	//{
	//	qglviewer::Vec tmpVector = camera()->upVector();
	//	_sampleCore->getShadowMap()->setCameraUp( glm::vec3( tmpVector.x, tmpVector.y, tmpVector.z ) );
	//}
	////_sampleCore->getShadowMap()->setCameraFovY( 45.f );
	////_sampleCore->getShadowMap()->setCameraAspectRatio( 1.f );
	////_sampleCore->getShadowMap()->setCameraZNear( 0.01f );
	////_sampleCore->getShadowMap()->setCameraZFar( 100.f );
	//_sampleCore->getShadowMap()->setCameraFovY( camera()->fieldOfView() * 180.f * 3.141592f );
	//_sampleCore->getShadowMap()->setCameraAspectRatio( camera()->aspectRatio() );
	//_sampleCore->getShadowMap()->setCameraZNear( camera()->zNear() );
	//_sampleCore->getShadowMap()->setCameraZFar( camera()->zFar() );

	////_sampleCore->getShadowMap()->setLightEye( glm::vec3( 8.f, 3.f, 6.f ) );
	//{
	//	float lightPosition[ 3 ];
	//	_light1->getPosition( lightPosition[ 0 ], lightPosition[ 1 ], lightPosition[ 2 ] );
	//	_sampleCore->getShadowMap()->setLightEye( glm::vec3( lightPosition[ 0 ], lightPosition[ 1 ], lightPosition[ 2 ] ) );
	//}
	////_sampleCore->getShadowMap()->setLightCenter( glm::vec3( 0.f, 3.f, 0.f ) );
	//{
	//	qglviewer::Vec tmpVector = camera()->sceneCenter();
	//	_sampleCore->getShadowMap()->setLightCenter( glm::vec3( tmpVector.x, tmpVector.y, tmpVector.z ) );
	//}
	//_sampleCore->getShadowMap()->setLightUp( glm::vec3( 0.f, 1.f, 0.f ) );
	//_sampleCore->getShadowMap()->setLightFovY( 45.f );
	//_sampleCore->getShadowMap()->setLightAspectRatio( 1.f );
	//_sampleCore->getShadowMap()->setLightZNear( 0.01f );
	//_sampleCore->getShadowMap()->setLightZFar( 100.f );
