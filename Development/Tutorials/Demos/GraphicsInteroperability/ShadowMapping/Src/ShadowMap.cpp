...
bool ShadowMap::initialize()
{
	...
	statusOK = _shaderProgramFirstPass->addShader( GvShaderProgram::eVertexShader, "D:\\Projects\\INRIA\\LastGV\\Release\\Bin\\Data\\Shaders\\ShadowMapping\\shadowMapping_1stPass_vert.glsl" );
	assert( statusOK );
	statusOK = _shaderProgramFirstPass->addShader( GvShaderProgram::eFragmentShader, "D:\\Projects\\INRIA\\LastGV\\Release\\Bin\\Data\\Shaders\\ShadowMapping\\shadowMapping_1stPass_frag.glsl" );
	assert( statusOK );
	...
	statusOK = _shaderProgramSecondPass->addShader( GvShaderProgram::eVertexShader, "D:\\Projects\\INRIA\\LastGV\\Release\\Bin\\Data\\Shaders\\ShadowMapping\\shadowMapping_2ndPass_vert.glsl" );
	assert( statusOK );
	statusOK = _shaderProgramSecondPass->addShader( GvShaderProgram::eFragmentShader, "D:\\Projects\\INRIA\\LastGV\\Release\\Bin\\Data\\Shaders\\ShadowMapping\\shadowMapping_2ndPass_frag.glsl" );
	assert( statusOK );
...
void ShadowMap::render( const float4x4& pModelViewMatrix, const float4x4& pProjectionMatrix, const int4& pViewport )
{
	...
	// Build shadow matrix
	shadowMatrix = sScaleBiasMatrix * lightModelViewMatrix * modelMatrix;
	// Set uniform(s)
