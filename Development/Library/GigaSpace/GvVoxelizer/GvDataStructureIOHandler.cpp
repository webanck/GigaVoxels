 ...
 * Empty node flag
 */
const unsigned int GvDataStructureIOHandler::_cEmptyNodeFlag = 0;
 ...
 ******************************************************************************/
GvDataStructureIOHandler::GvDataStructureIOHandler( const std::string& pName, 
							unsigned int pLevel,
							unsigned int pBrickWidth,
							GvDataTypeHandler::VoxelDataType pDataType,
							bool pNewFiles )
// TO DO
...
GvDataStructureIOHandler::GvDataStructureIOHandler( const std::string& pName, 
							unsigned int pLevel,
							unsigned int pBrickWidth,
							const vector< GvDataTypeHandler::VoxelDataType >& pDataTypes,
							bool pNewFiles )
// TO DO
// attention à l'ordre des initializations...
:	_nodeFile( NULL )
 ...
 * ...
 ******************************************************************************/
GvDataStructureIOHandler::~GvDataStructureIOHandler()
{
	if ( _isBufferLoaded )
	{
		saveNodeandBrick();
 ...
 ******************************************************************************/
unsigned int GvDataStructureIOHandler::getBrickNumber() const
{
	return _brickNumber;
}
 ...
 ******************************************************************************/
float GvDataStructureIOHandler::getVoxelSize() const
{
 ...
 ******************************************************************************/
void GvDataStructureIOHandler::setVoxel( unsigned int pVoxelPos[ 3 ], void* pVoxelData, unsigned int pDataChannel )
{
	...
	if( isEmpty( _nodeBuffer ) )
	{
		...
		for ( unsigned int i = 0; i < _dataTypes.size(); ++i )
		{
			...
			fwrite( _brickBuffers[ i ], GvDataTypeHandler::canalByteSize( _dataTypes[ i ] ), _brickSize, _brickFiles[ i ] );
	...
	// Write voxel data
	memcpy( GvDataTypeHandler::getAddress( _dataTypes[ pDataChannel ], _brickBuffers[ pDataChannel ], voxelPosInBrick[ 0 ] + ( _brickWidth + 2 ) * ( voxelPosInBrick[ 1 ] + ( _brickWidth + 2 ) * voxelPosInBrick[ 2 ] ) ),
			pVoxelData,
			GvDataTypeHandler::canalByteSize( _dataTypes[ pDataChannel ] ) );
}
 ...
 * @param pDataChannel data channel index
 ******************************************************************************/
void GvDataStructureIOHandler::setVoxel( float pNormalizedVoxelPos[ 3 ], void* pVoxelData, unsigned int pDataChannel )
{
 ...
 * @param pDataChannel data channel index
 ******************************************************************************/
void GvDataStructureIOHandler::getVoxel( unsigned int pVoxelPos[ 3 ], void* voxelData, unsigned int pDataChannel )
{
	// Retrieve the associated node position in which the voxel resides
	unsigned int nodePos[ 3 ];
	nodePos[ 0 ] = pVoxelPos[ 0 ] / _brickWidth;
	...
	memcpy( voxelData,
			GvDataTypeHandler::getAddress( _dataTypes[ pDataChannel ], _brickBuffers[ pDataChannel ], voxelPosInBrick[ 0 ] + ( _brickWidth + 2 ) * ( voxelPosInBrick[ 1 ] + ( _brickWidth + 2 ) * voxelPosInBrick[ 2 ] ) ),		
			GvDataTypeHandler::canalByteSize( _dataTypes[ pDataChannel ] ) );
}
 ...
 * @param pDataChannel data channel index
 ******************************************************************************/
void GvDataStructureIOHandler::getVoxel( float pNormalizedVoxelPos[ 3 ], void* pVoxelData, unsigned int pDataChannel )
{
	// Retrieve the indexed voxel position
	unsigned int voxelPos[ 3 ];
	getVoxelPosition( pNormalizedVoxelPos, voxelPos );
 ...
 * @param pDataChannel data channel index
 ******************************************************************************/
void GvDataStructureIOHandler::getBrick( unsigned int pNodePos[ 3 ], void* pBrickData, unsigned int pDataChannel )
{
	// Retrieve node info and associated brick data
	loadNodeandBrick( pNodePos );

	// Read data from memory
	memcpy( pBrickData, _brickBuffers[ pDataChannel ], _brickSize * GvDataTypeHandler::canalByteSize( _dataTypes[ pDataChannel ] ) );
}
 ...
 * @param pDataChannel data channel index
 ******************************************************************************/
void GvDataStructureIOHandler::setBrick( unsigned int pNodePos[ 3 ], void* pBrickData, unsigned int pDataChannel )
{
	// Retrieve node info and associated brick data
	loadNodeandBrick( pNodePos );

	// Write data in memory
	memcpy( _brickBuffers[ pDataChannel ], pBrickData, _brickSize * GvDataTypeHandler::canalByteSize( _dataTypes[ pDataChannel ] ) );
}
 ...
 * @param pNodePos indexed node position
  ******************************************************************************/
void GvDataStructureIOHandler::getNodePosition( float pNormalizedNodePos[ 3 ], unsigned int pNodePos[ 3 ] )
{
 ...
 ******************************************************************************/
void GvDataStructureIOHandler::getVoxelPosition( float pNormalizedVoxelPos[ 3 ], unsigned int pVoxelPos[ 3 ] )
{
 ...
 ******************************************************************************/
void GvDataStructureIOHandler::getVoxelPositionInBrick( float pNormalizedVoxelPos[ 3 ], unsigned int pVoxelPosInBrick[ 3 ] )
{
 ...
 ******************************************************************************/
unsigned int GvDataStructureIOHandler::getNode( unsigned int pNodePos[ 3 ] )
{
	// Retrieve node info and associated brick data
	loadNodeandBrick( pNodePos );
 ...
 * @param pNodePos node position
 ******************************************************************************/
void GvDataStructureIOHandler::loadNodeandBrick( unsigned int pNodePos[ 3 ] )
{
	...
	for ( unsigned int c = 0; c < _dataTypes.size(); ++c )
	{
		...
		if ( isEmpty( _nodeBuffer ) )
		{
			memset( _brickBuffers[ c ], 0, _brickSize * GvDataTypeHandler::canalByteSize( _dataTypes[ c ] ) );
		}
		else
		{
...
#ifdef WIN32
			__int64 offset = (__int64)brickOffset*(__int64)_brickSize*(__int64)GvDataTypeHandler::canalByteSize( _dataTypes[ c ] );
			_fseeki64( _brickFiles[c], offset, SEEK_SET );
#else
			fseek( _brickFiles[ c ], brickOffset * _brickSize * GvDataTypeHandler::canalByteSize( _dataTypes[ c ] ), SEEK_SET );
#endif			
			fread( _brickBuffers[ c ], GvDataTypeHandler::canalByteSize( _dataTypes[ c ] ), _brickSize, _brickFiles[ c ] );
		}
	}
 ...
 ******************************************************************************/
void GvDataStructureIOHandler::saveNodeandBrick()
{
	...
	if ( _isBufferLoaded )
	{
		...
		for ( unsigned int c = 0; c < _dataTypes.size(); ++c )
		{
			...
			fseek( _brickFiles[ c ], brickOffset * _brickSize * GvDataTypeHandler::canalByteSize( _dataTypes[ c ] ), SEEK_SET );
#endif
			fwrite( _brickBuffers[ c ], GvDataTypeHandler::canalByteSize( _dataTypes[ c ] ), _brickSize, _brickFiles[ c ] );
 ...
 ******************************************************************************/
void GvDataStructureIOHandler::computeBorders()
{
	// LOG message
	std::cout << "GvDataStructureIOHandler::computeBorders()" << std::endl;
	
	// Iterate trough data ytpes
	for ( unsigned int c = 0; c < _dataTypes.size(); ++c )
	{
		// Create two brick buffers
		void* brick = GvDataTypeHandler::allocateVoxels( _dataTypes[ c ], _brickSize );
		void* brick2 = GvDataTypeHandler::allocateVoxels( _dataTypes[ c ], _brickSize );
				...
				for ( unsigned int x = 1; x < _brickWidth + 1; ++x )
				{
					...
					int z2 = (-k2) * (_brickWidth) + z;
							...
							&& z2 >= 0 && z2 <= static_cast< int >( _brickWidth ) + 1 ) )
						{
							...
							memcpy(
									/*destination*/GvDataTypeHandler::getAddress( _dataTypes[ c ], brick2, x2 + ( _brickWidth + 2 ) * ( y2 + ( _brickWidth + 2 ) * z2 ) ),
									/*source*/GvDataTypeHandler::getAddress( _dataTypes[ c ], brick , x  + ( _brickWidth + 2 ) * ( y  + ( _brickWidth + 2 ) * z ) ),
									/*size*/GvDataTypeHandler::canalByteSize( _dataTypes[ c ] )
							);
 ...
 * @param pNewFiles a flag telling wheter or not new files are used
 ******************************************************************************/
void GvDataStructureIOHandler::openFiles( const string& pName, bool pNewFiles )
{
	...
	for ( int c = 0; c < _dataTypes.size(); ++c )
	{
		...
		_fileNamesBrick.push_back( getFileNameBrick( pName, _level, _brickWidth, c, GvDataTypeHandler::getTypeName( _dataTypes[ c ] ) ) );
		...
		_brickBuffers.push_back( GvDataTypeHandler::allocateVoxels( _dataTypes[ c ], _brickSize ) );
		memset( _brickBuffers[ c ], 0, _brickSize * GvDataTypeHandler::canalByteSize( _dataTypes[ c ] ) );
	}
}
 ...
 ******************************************************************************/
string GvDataStructureIOHandler::getFileNameNode( const std::string& pName, unsigned int pLevel, unsigned int pBrickWidth )
{
 ...
 ******************************************************************************/
string GvDataStructureIOHandler::getFileNameBrick( const string& pName, unsigned int pLevel, unsigned int pBrickWidth, unsigned int pDataChannelIndex, const string& pDataTypeName )
{
	std::ostringstream oss;
 ...
 ******************************************************************************/
bool GvDataStructureIOHandler::isEmpty( unsigned int pNode )
{
 ...
 ******************************************************************************/
unsigned int GvDataStructureIOHandler::getBrickOffset( unsigned int pNode )
{
