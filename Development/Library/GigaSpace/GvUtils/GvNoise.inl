...
GvNoise::~GvNoise()
{
	...
	GV_CUDA_SAFE_CALL( cudaFreeArray( _gradientCUDAArray ) );
}
 ...
 ******************************************************************************/
inline bool GvNoise::initialize()
{
	/**
...

	{
		...
		GV_CUDA_SAFE_CALL( cudaMallocArray( &_dataArray, &channelFormatDesc, nElem, nElem ) );

		// Copy the lookup table to device memory
		GV_CUDA_SAFE_CALL( cudaMemcpy2DToArrayAsync( _dataArray, 0, 0, tmpTable, nElem * sizeof( uchar4 ), nElem * sizeof( uchar4 ), nElem, cudaMemcpyHostToDevice ));

		// Bind the array to a texture
		
		GV_CUDA_SAFE_CALL( cudaBindTextureToArray( gs_permutationTableTexture, _dataArray ) );
	...
	GV_CHECK_CUDA_ERROR( "GvUtils::GvNoise::initialize" );

	{
		...
		GV_CUDA_SAFE_CALL( cudaMallocArray( &_gradientCUDAArray, &channelFormatDesc, gradientTableNElem ) );
		...
		GV_CUDA_SAFE_CALL( cudaBindTextureToArray( gs_gradientTexture, _gradientCUDAArray ) );
