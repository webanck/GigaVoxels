...
void RenderKernelSimple( TVolTreeKernelType pVolumeTree, TCacheType pCache )
{
	...
	GvRendererKernel::initPixelCoords< TBlockResolution >( Pid, /*blockPos,*/ pixelCoords );
	...
	//if ( inFrame )
	{
		...
		float frameDepth = getInputDepth( pixelCoords );		// TO DO : this read memory could be placed before to avoid cycles waiting...
		...
		int hit = intersectBox( rayStartInTree, rayDirInTree, make_float3( 0.001f ), make_float3( 0.999f ), boxInterMin, boxInterMax );
		...
		if ( ! masked )
		{
			...
			GvRendererKernel::render< TFastUpdateMode, TPriorityOnBrick >( pVolumeTree, sampleShader, pCache, pixelCoords, rayStartInTree, rayDirInTree, tMax, t );
